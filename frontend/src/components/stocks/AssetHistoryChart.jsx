...
const AssetHistoryChart = ({ ticker }) => {
  ...
  const chartData = useMemo(() => {
    ...
    return history.map((h) => {
      const buyTx = transactions.find(
        (t) => t.date === h.date && t.type === "buy",
      );
      const sellTx = transactions.find(
        (t) => t.date === h.date && t.type === "sell",
      );
  ...
  const allValues = [...prices, average_buy_price].filter(
  ...
  return (
    <div
      className="fm-card"
      style={{ padding: "24px", boxShadow: "0 10px 40px rgba(0,0,0,0.1)" }}
    >
      <div
        style={{
          display: "flex",
      ...
      >
        <div>
          ...
          <div
            style={{
              color:
                price_diff_perc >= 0 ? "var(--green-500)" : "var(--red-500)",
              fontWeight: "bold",
              fontSize: "var(--font-size-sm)",
            }}
          >
            {price_diff_perc >= 0 ? "+" : ""}
            {(price_diff_perc * 100).toFixed(2)}% vs Costo Promedio
          </div>
        </div>
          ...
          {RANGE_OPTIONS.map((opt) => (
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                backgroundColor:
                  timeRange === opt.value ? "var(--white)" : "transparent",
                color:
                  timeRange === opt.value
                    ? "var(--blue-600)"
                    : "var(--gray-500)",
          ...
          >
            ...
            <Tooltip
              content={({ active, payload, label }) => {
                ...
                // label ya es datetime del backend, parsear directamente
                const [year, month, day] = label.split("-");
                const dateStr = `${parseInt(day)} ${["enero", "febrero", "marzo", "abril", "mayo", "junio", "julio", "agosto", "septiembre", "octubre", "noviembre", "diciembre"][parseInt(month) - 1]} ${parseInt(year)}`;
                return (
