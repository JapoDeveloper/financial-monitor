  ...
  const CustomPieTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      return (
        ...
        >
          <span style={{ fontWeight: 700, color: "var(--blue-800)" }}>
            {payload[0].name}
          </span>
  ...
  return (
    ...
    >
      ...
      {payload.map((p) => (
        ...
        >
          <div
            style={{ width: 8, height: 8, borderRadius: 2, background: p.fill }}
          />
...
const PortfolioHoldings = () => {
  ...
  const usdRate =
    usdMoneda && usdMoneda.total_valor_nominal > 0
      ? usdMoneda.total_valor / usdMoneda.total_valor_nominal
      : 58.0;

  const totalRVDOP =
    data?.distribucion_clase?.find(
      (c) => c.nombre?.toLowerCase() === "renta variable",
    )?.total_valor || 0;
  ...
  if (data?.renta_variable) {
    ...
    if (rebalanceMode === "total") {
      ...
      if (rebalanceMode === "compra-total") {
        ...
        rawAssets.forEach((item) => {
          if (item.instrumento === "Cash / Equivalents") {
            ...
            refTargetWeights[item.instrumento] =
              item.meta_objetivo / (1 - cashTargetWeight);
          }
        });
      } else {
      ...
      rawAssets.forEach((item) => {
        if (item.instrumento !== "Cash / Equivalents") {
          ...
          deficits.push({
            ...
            targetValue,
            deficit,
            refWeight: refTargetWeights[item.instrumento],
          });
      ...
      if (cashToSpend < totalDeficit) {
        ...
        deficits.forEach((d) => {
          allocations[d.instrumento] =
            totalDeficit > 0 ? cashToSpend * (d.deficit / totalDeficit) : 0;
        });
      } else {
        ...
        deficits.forEach((d) => {
          ...
          const scaleFactor =
            rebalanceMode === "compra-total" ? 1 : 1 - cashTargetWeight;
  ...
  return (
    <div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 20px" }}>
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title">Tenencias del Portafolio</h1>
          <p className="page-subtitle">
            Distribución y análisis detallado de activos — valores ajustados a
            moneda base
          </p>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <p className="kpi-value" style={{ color: "var(--blue-800)" }}>
                {fmtCurrency(
                  item.total_valor_nominal || item.total_valor,
                  item.nombre,
                )}
              </p>
            </div>
      ...
      <div className="section-spacing">
        <CategoryExplorer
          distributions={[
            ...
            {
              data: data.distribucion_institucion,
                      ...
                      const grouped = Object.values(
                        subclase.children.reduce((acc, inst) => {
                          acc[inst.name] = acc[inst.name]
                            ? {
                                ...acc[inst.name],
                                value: acc[inst.name].value + inst.value,
                              }
                            : { ...inst };
                          return acc;
                        }, {}),
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {((subclase.value / clase.value) * 100).toFixed(
                                1,
                              )}
                              %
                            </p>
                            ...
                            >
                              ...
                              >
                                ...
                                <div style={{ textAlign: "right" }}>
                                  ...
                                  >
                                    {((inst.value / clase.value) * 100).toFixed(
                                      1,
                                    )}
                                    %
                                  </p>
      ...
      <div className="fm-card section-spacing">
        <h3
          ...
          style={{ fontSize: "var(--font-size-md)", marginBottom: "20px" }}
        >
          Renta Variable — Meta vs. Actual
        </h3>
        <ResponsiveContainer width="100%" height={400}>
          <BarChart
            data={[...data.renta_variable].sort(
          ...
          >
            ...
            <XAxis
              ...
              tick={(props) => {
                ...
                const item = [...data.renta_variable].find(
                  (d) => d.instrumento === payload.value,
                );
                const isAligned =
                  item &&
                  Math.abs(item.valor_actual - item.meta_objetivo) <= 0.01;
                return (
                  <text
                    x={x}
                    y={y + 15}
                    ...
                    fill={isAligned ? CHART_PALETTE[0] : "var(--gray-600)"}
                    transform={`rotate(-10, ${x}, ${y + 15})`}
                  >
                    {payload.value}
                  </text>
                );
      ...
      <div className="fm-card section-spacing">
        <div className="rebalance-panel-header">
          <h3 className="section-title">
            Simulador de Rebalanceo — Renta Variable
          </h3>
        ...
        <div className="rebalance-info-card animate-fade-in">
          {rebalanceMode === "compra-excedente" && (
            <p>
              <strong>Modalidad: Solo Compra (Efectivo Excedente).</strong> Se
              utiliza únicamente el efectivo de <code>Cash / Equivalents</code>{" "}
              que supera su peso meta de{" "}
              <strong>{(cashTargetWeight * 100).toFixed(1)}%</strong>{" "}
              (equivalente a{" "}
              <strong>{fmtCurrency(totalRV * cashTargetWeight, "USD")}</strong>{" "}
              de reserva mínima). Se compra proporcionalmente los activos
              subponderados sin vender ningún activo. Efectivo a distribuir:{" "}
              <strong>
                {fmtCurrency(
                  Math.max(0, cashActualValue - totalRV * cashTargetWeight),
                  "USD",
                )}
              </strong>
              .
          ...
          {rebalanceMode === "compra-total" && (
            <p>
              <strong>Modalidad: Solo Compra (Todo el Cash).</strong> Se
              distribuye la totalidad del efectivo disponible en{" "}
              <code>Cash / Equivalents</code> (
              <strong>{fmtCurrency(cashActualValue, "USD")}</strong>) para
              comprar proporcionalmente los activos subponderados sin vender
              ningún activo. El peso proyectado de caja será{" "}
              <strong>0.0%</strong>.
            </p>
          )}
          {rebalanceMode === "total" && (
            <p>
              <strong>Modalidad: Rebalanceo Total.</strong> Ajuste institucional
              completo. Se venden los activos sobreponderados y se compran los
              subponderados. La reserva de <code>Cash / Equivalents</code> se
              ajusta exactamente a su peso meta de{" "}
              <strong>{(cashTargetWeight * 100).toFixed(1)}%</strong> (
              <strong>{fmtCurrency(totalRV * cashTargetWeight, "USD")}</strong>
              ).
            </p>
        ...
        {filteredSimulatedAssets.length > 0 ? (
                  ...
                  return (
                    <tr key={item.instrumento}>
                      ...
                      <td className="col-right">
                        {fmtCurrency(item.valorActual, "USD")}
                      </td>
                      <td className="col-right">
                        {(item.pesoActual * 100).toFixed(2)}%
                      </td>
                      <td className="col-center">
                        ...
                        {isSell && (
                          <span className="badge-transaction badge-transaction--sell">
                            Venta: -
                            {fmtCurrency(Math.abs(item.deltaValue), "USD")}
                          </span>
                        )}
                      </td>
                      <td
                        className={`col-right fw-bold ${Math.abs(item.pesoProyectado - item.pesoMeta) <= 0.01 ? "text-positive" : "text-primary"}`}
                      >
          ...
          <div className="empty-state">
            <p>
              El portafolio se encuentra completamente al día con los pesos meta
              para esta modalidad.
            </p>
          </div>
        )}
        ...
        <div className="rebalance-summary">
          <div className="rebalance-summary-item">
            <span className="rebalance-summary-label">
              Total Sub-portafolio RV:
            </span>
            <span>{fmtCurrency(totalRV, "USD")}</span>
          </div>
          <div className="rebalance-summary-item">
            <span className="rebalance-summary-label">
              Efectivo Post-Rebalanceo:
            </span>
            <span>
              {fmtCurrency(
                (simulatedAssets.find(
                  (item) => item.instrumento === "Cash / Equivalents",
                )?.pesoProyectado || 0) * totalRV,
                "USD",
              )}
            </span>
          </div>
