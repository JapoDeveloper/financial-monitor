...
import { DollarSign, Coins, TrendingUp, TrendingDown, Loader2, BarChart3, TableProperties } from 'lucide-react';
import { CHART_PALETTE, CURRENCY_COLORS, TOOLTIP_STYLE, SELECTOR_STYLES } from '../constants/chartTheme';
...
const ChartTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{
      background: '#fff', border: '1px solid #E8EEF8', borderRadius: 10,
      padding: '10px 14px', boxShadow: '0 4px 16px rgba(30,58,138,0.1)',
      ...
      {payload.map((p) => (
        <div key={p.name} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
          ...
          <span style={{ fontWeight: 700, color: p.value >= 0 ? 'var(--green-500)' : 'var(--red-500)' }}>
            {`${(p.value).toFixed(2)}%`}
          </span>
        </div>
      ))}
...
const SummaryCard = ({ currencyCode, data, index }) => {
  ...
  return (
    <div className="fm-card fm-card--accent" style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center',}}>
          <div style={{
            ...
            {currencyCode === 'USD' ? <DollarSign size={18} /> : <Coins size={18} />}
          </div>
          <div>
            <p className="kpi-label" style={{ marginBottom: 0 }}>Resumen {currencyCode}</p>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div>
          <p style={{ fontSize: 11, color: 'var(--gray-400)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 3 }}>Valor Actual</p>
          <p style={{ fontSize: 20, fontWeight: 800, color: 'var(--gray-800)' }}>
            {fmtCompact(data.summary.current_value)} <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--gray-400)' }}>{currencyCode}</span>
          </p>
        </div>
        <div>
          <p style={{ fontSize: 11, color: 'var(--gray-400)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 3 }}>Ganancia Total</p>
          <p style={{ fontSize: 20, fontWeight: 800, color: data.summary.total_gain >= 0 ? 'var(--green-500)' : 'var(--red-500)' }}>
...
const ConsolidatedMonthlyTable = ({ yearlyData, currencies }) => {
  const periods = yearlyData[currencies[0]]?.monthly_breakdown.map(b => b.period) || [];

  return (
    <div className="fm-card" style={{ padding: 0, overflow: 'hidden', marginBottom: 24 }}>
        ...
        <table className="fm-table">
          <thead>
            <tr>
              <th>Período</th>
              {currencies.map(c => (
                <th key={c} colSpan={6} style={{
                  textAlign: 'center',
                  background: CURRENCY_COLORS[c] ? `${CURRENCY_COLORS[c].area}10` : undefined,
            ...
            <tr>
              ...
              {currencies.map(c => (
                <React.Fragment key={c}>
                  {['Saldo I.', 'Flujo', 'Ganancia', 'Saldo F.', 'Ret.', 'Acum.'].map(h => (
                    <th key={h} style={{
                      textAlign: 'right', fontWeight: 600,
                      background: CURRENCY_COLORS[c] ? `${CURRENCY_COLORS[c].area}08` : undefined,
                    }}>{h}</th>
                  ))}
                </React.Fragment>
              ))}
          ...
          <tbody>
            {periods.map((period, pIndex) => (
              <tr key={period}>
                <td style={{ fontWeight: 600, color: 'var(--blue-800)', whiteSpace: 'nowrap' }}>{period}</td>
                {currencies.map((c, cIdx) => {
                  const row = yearlyData[c].monthly_breakdown[pIndex];
                  const borderLeft = cIdx > 0 ? `2px solid ${CURRENCY_COLORS[c]?.area || '#E8EEF8'}30` : undefined;
                  const fmt0 = (v) => new Intl.NumberFormat('en-US', { minimumFractionDigits: 0 }).format(v);
                  return (
                    <React.Fragment key={c}>
                      <td style={{ textAlign: 'right', borderLeft }}>{fmt0(row.v_inicial)}</td>
                      <td style={{ textAlign: 'right' }}>{fmt0(row.flujo_neto)}</td>
                      <td style={{ textAlign: 'right' }}>{fmt0(row.ganancia_mercado)}</td>
                      <td style={{ textAlign: 'right', fontWeight: 600 }}>{fmt0(row.valor_cartera)}</td>
                      <td style={{ textAlign: 'right', fontWeight: 700, color: row.retorno_mensual >= 0 ? 'var(--green-500)' : 'var(--red-500)' }}>
                        {pct(row.retorno_mensual)}
                      </td>
                      <td style={{ textAlign: 'right', fontWeight: 700, color: row.retorno_acc >= 0 ? 'var(--blue-600)' : 'var(--red-500)' }}>
                        {pct(row.retorno_acc)}
                      </td>
                    </React.Fragment>
                  );
...
const ProfitabilityEvolution = ({ year, initialCurrency }) => {
  ...
  if (isLoading) return <div className="fm-spinner"><Loader2 size={24} style={{ animation: 'spin 1s linear infinite' }} /></div>;
    ...
    <div className="fm-card" style={{ marginBottom: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <div style={SELECTOR_STYLES.container}>
          {['DOP', 'USD'].map(c => (
            <button key={c} onClick={() => setCurrency(c)} style={SELECTOR_STYLES.button(currency === c)}>
              {c}
            </button>
          ))}
        </div>
      ...
      <ResponsiveContainer width="100%" height={420}>
        <LineChart data={chartData} margin={{ top: 10, right: 20, left: 10, bottom: 20 }}>
          ...
          <Line
            ...
            strokeOpacity={hoveredSeries !== null ? (hoveredSeries === 'PROMEDIO' ? 1 : 0.15) : 1}
            strokeDasharray="5 5"
            dot={{ r: hoveredSeries === 'PROMEDIO' ? 5 : 4, fill: 'var(--gray-400)' }}
            activeDot={{ r: 6 }}
...
const CagrTable = () => {
  ...
  if (isLoading) return <div className="fm-spinner"><Loader2 size={32} style={{ animation: 'spin 1s linear infinite' }} /></div>;
  ...
  return (
    <div className="fm-card" style={{ padding: 0, overflow: 'hidden', marginBottom: 24 }}>
      <div style={{ padding: '18px 24px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {currencies.map(curr => (
              <React.Fragment key={curr}>
                <tr style={{ background: 'var(--gray-50)' }}>
                  <td colSpan={9} style={{ fontWeight: 800, color: 'var(--blue-800)', fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.05em', padding: '8px 24px' }}>
                    Bloque {curr}
                  </td>
                </tr>
                {data.filter(d => d.currency === curr).map((row, idx) => (
                  <tr key={`${row.instrument}-${idx}`}>
                    <td style={{ fontWeight: 600, color: 'var(--gray-700)' }}>{row.instrument}</td>
                    <td style={{ textAlign: 'center' }}>
                       <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 6px', borderRadius: 4, background: CURRENCY_COLORS[row.currency]?.area + '20', color: CURRENCY_COLORS[row.currency]?.area }}>
                        {row.currency}
                       </span>
                    </td>
                    <td style={{ textAlign: 'right', fontWeight: 700, color: 'var(--blue-600)' }}>{row.weight.toFixed(2)}%</td>
                    {[row.cagr_30d, row.cagr_90d, row.cagr_180d, row.cagr_1y, row.cagr_3y, row.cagr_5y].map((v, i) => (
                      <td key={i} style={{ 
                        textAlign: 'right', 
                        fontWeight: 700, 
                        color: v === null ? 'var(--gray-300)' : v >= 0 ? 'var(--green-500)' : 'var(--red-500)' 
                      }}>
                        {v !== null ? `${v.toFixed(2)}%` : '—'}
                      </td>
                    ))}
...
const PortfolioAnuallyPerformance = () => {
  ...
  const { data: historyData, isLoading: loadingHistory } = useQuery({
    ...
    queryFn: () => getPerformanceHistory(currentYear - selectedHistoryYears, currentYear),
  });
  ...
  let chartData = (historyData || []).map(h => ({ year: h.year, ...h.returns_by_currency }));
    ...
    <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ fontSize: 'var(--font-size-base)', color: 'var(--gray-400)' }}>
            Comparativa institucional de rentabilidades y evolución por divisa
          </p>
        </div>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          <div>
            <label className="fm-label" style={{ marginBottom: 4 }}>Año de análisis</label>
            <div className="fm-select-wrapper">
              <select className="fm-select" style={{ width: 'auto', minWidth: 100 }}
          ...
          <div>
            <label className="fm-label" style={{ marginBottom: 4 }}>Historial</label>
            <div className="fm-select-wrapper">
              <select className="fm-select" style={{ width: 'auto', minWidth: 120 }}
                value={selectedHistoryYears} onChange={e => setSelectedHistoryYears(parseInt(e.target.value))}>
                {[3, 5, 7, 10].map(y => <option key={y} value={y}>Últimos {y} años</option>)}
              </select>
            </div>
          </div>
        </div>
      ...
      {/* ── Summary Cards ───────────────────────── */}
      {loadingYearly ? (
        <div className="fm-spinner"><Loader2 size={32} style={{ animation: 'spin 1s linear infinite', color: 'var(--blue-500)' }} /></div>
      ) : (
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 24 }}>
          {currencies.map((currency, i) => (
            <SummaryCard key={currency} currencyCode={currency} data={yearlyData.data_by_currency[currency]} index={i} />
          ))}
        </div>
      )}
      ...
      {!loadingYearly && yearlyData && currencies.length > 0 && (
        <ConsolidatedMonthlyTable yearlyData={yearlyData.data_by_currency} currencies={currencies} />
      )}

      {/* ── Profitability Evolution Chart (New) ─── */}
      <ProfitabilityEvolution year={selectedYear} initialCurrency={currencies[0]} />
      ...
      <div className="fm-card" style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h3 style={{ fontSize: 'var(--font-size-md)', fontWeight: 700, color: 'var(--gray-800)' }}>
            Rendimientos Históricos de Cartera
          </h3>
          <span style={{ fontSize: 'var(--font-size-xs)', color: 'var(--gray-400)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
            Retorno anual acumulado por divisa
          </span>
        </div>

        {loadingHistory ? (
          <div className="fm-spinner"><Loader2 size={28} style={{ animation: 'spin 1s linear infinite', color: 'var(--blue-500)' }} /></div>
        ) : (
          <ResponsiveContainer width="100%" height={340}>
            <AreaChart data={chartData} margin={{ top: 10, right: 20, left: 10, bottom: 0 }}>
              ...
              <CartesianGrid strokeDasharray="3 3" stroke="#E8EEF820" vertical={false} />
              <XAxis dataKey="year" tick={{ fontSize: 12, fill: '#9CA3AF', fontFamily: 'var(--font-family)' }} axisLine={false} tickLine={false} />
              <YAxis tickFormatter={pct} tick={{ fontSize: 11, fill: '#9CA3AF', fontFamily: 'var(--font-family)' }} axisLine={false} tickLine={false} width={52} />
              <ReferenceLine y={0} stroke="#E8EEF8" strokeWidth={1.5} />
              <Tooltip content={<ChartTooltip />} />
              <Legend
                wrapperStyle={{ fontSize: 12, fontFamily: 'var(--font-family)', fontWeight: 600, paddingTop: 16, cursor: 'pointer' }}
